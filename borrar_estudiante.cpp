...
void borrar_estudiante() {
    ifstream infile("datos_estudiantes(2).csv");
    ofstream outfile("datos_estudiantes(2)_auxiliar.csv");
    if (!infile || !outfile) {
        ...
        while (getline(infile, linea)) {
            ...
            if (!(stoi(id) == id_buscar)) {
                outfile << id << "," << nombre << "," << apellido << "," << genero << "," << email << "," << seccion << "," << matematicas
                         << "," << sociales << "," << biologia << "," << educ_fisica << "," << artes << endl;
            }else{
